...
export const initialState = { f: "10*sin(x)", a: 0, b: 10, N: 100 };
export type IState = typeof initialState;
export type IActions = {
  update<S extends Partial<S>>(s: S): S;
};
...
export function Graph({ state, actions: { update } }: IGraphProps) {
  const { f, a, b, N } = state;
  let graph: any;
  try {
